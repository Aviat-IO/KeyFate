...
async function seedDatabase() {
  ...
  try {
    ...
    const freeTier = await client.query(
      `SELECT id FROM tiers WHERE name = 'free' LIMIT 1`,
    )
    const proTier = await client.query(
      `SELECT id FROM tiers WHERE name = 'pro' LIMIT 1`,
    )
    ...
    await client.query(`
      ...
      ('11111111-1111-1111-1111-111111111111', 'dev@localhost', crypt('DevPass_c6e7a3f3c9a98b55', gen_salt('bf')), NOW(), NOW(), NOW()),
      ('22222222-2222-2222-2222-222222222222', 'test@localhost', crypt('DevPass_c6e7a3f3c9a98b55', gen_salt('bf')), NOW(), NOW(), NOW())
    ...
    const secretCount = await client.query(
  ...
  console.log("")
  console.log("⚠️  WARNING: These are DEVELOPMENT-ONLY credentials!")
  console.log("🔑 Development Login Credentials:")
  console.log("   📧 dev@localhost / DevPass_c6e7a3f3c9a98b55 (Free tier)")
  console.log("   📧 test@localhost / DevPass_c6e7a3f3c9a98b55 (Pro tier)")
  console.log("")
  console.log("⚠️  NEVER use these credentials in staging or production!")
}

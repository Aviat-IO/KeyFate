/**
 * Server-side environment variables for SvelteKit
 *
 * Uses $env/dynamic/private for runtime env access (not baked in at build time).
 * This file should only be imported in server-side code (+page.server.ts, +server.ts, hooks.server.ts, etc.)
 */
...
function getBTCPayServerUrl(): string {
  const url = env.BTCPAY_SERVER_URL
  if (!url) {

 ...
 *
 * Ported from frontend/__tests__/utils/test-factories.ts
 * Adapted for SvelteKit (no Next.js auth types).
 */
...
/**
 * Create a mock auth session response (SvelteKit Auth.js style)
 */
...
export function createMockApiError(message: string, status: number = 500) {
	return createMockApiResponse({ error: message }, { status, ok: false, statusText: message });
}

/**
 * Create a mock toast function (svelte-sonner style)
 */
...
/**
 * Create a mock SvelteKit goto function
 */

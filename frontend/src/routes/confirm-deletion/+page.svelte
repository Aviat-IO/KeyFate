<script lang="ts">
	...
	$effect(() => {
		...
		if (!token) {
			confirmationState = 'invalid_token';
			return;
		}
			...
			.then(async (response) => {
				...
				if (!response.ok) {
					if (data.error?.includes('already')) {
						confirmationState = 'already_processed';
					} else {
						confirmationState = 'error';
						errorMessage = data.error || 'Failed to confirm deletion';
					}
					return;
				}
			...
			{:else if confirmationState === 'success'}
				<div class="bg-destructive/10 border-destructive/50 rounded-lg border p-4">
					...
					<p class="text-sm">
						...
						{#if scheduledDate}<strong>{formatDate(scheduledDate)}</strong>{/if}. You can cancel
						this request at any time before that date.
					</p>
				</div>
				<div class="flex gap-2">
					<Button variant="outline" onclick={() => goto('/settings/privacy')} class="flex-1"

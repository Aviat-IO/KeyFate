...
<div class="bg-background min-h-screen">
	<nav
		class="border-border/50 bg-background/80 supports-[backdrop-filter]:bg-background/60 fixed top-0 right-0 left-0 z-50 border-b backdrop-blur-md"
	>
		<NavBar />
	</nav>
	...
	<!-- Hero -->
	<section class="relative container mx-auto overflow-hidden px-4 py-32">
	...
	<section class="bg-muted/30 container mx-auto px-4 py-24">
		...
		<div class="mx-auto grid max-w-6xl gap-8 md:grid-cols-3">
			{#each [{ icon: AlertTriangle, title: 'Keys Lost = Crypto Lost', desc: 'There\'s no "forgot password" for Bitcoin', content: "Unlike a bank account, no one can reset your crypto wallet. If your private keys die with you, your family's inheritance is gone — permanently and irreversibly." }, { icon: XCircle, title: 'Paper Backups Fail', desc: 'Fire, theft, and confusion destroy paper', content: 'Seed phrases on paper can burn, get stolen, or simply confuse non-technical family members. Even a safe deposit box requires someone to know it exists.' }, { icon: EyeOff, title: 'Trusting Others Is Risky', desc: 'Sharing keys means giving up control', content: 'Giving your private keys to a lawyer, friend, or family member means they could access your crypto at any time. You need a solution that releases access only when needed.' }] as card}
				<Card.Root
					class="group border-destructive/20 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
				>
					<Card.Header>
						<card.icon
							class="text-destructive mb-4 h-12 w-12 transition-transform duration-300 group-hover:scale-110"
						/>
	...
	<section id="how-it-works" class="container mx-auto px-4 py-24">
		<div class="mb-16 text-center">
			...
			<p class="text-muted-foreground mx-auto max-w-3xl text-xl">
				Three simple steps. Military-grade cryptography. Zero trust required.
			</p>
		</div>

		<div class="mx-auto grid max-w-6xl gap-8 lg:grid-cols-3">
			{#each [{ icon: Lock, step: 'Step 1', title: 'Create & Encrypt', desc: "Enter your seed phrase or private key. It's split into 3 encrypted pieces using Shamir's Secret Sharing — entirely in your browser." }, { icon: Shield, step: 'Step 2', title: 'Store Securely', desc: 'We store just 1 encrypted piece (useless alone). You keep 1 piece. Your recipient gets 1 piece. Nobody has enough to reconstruct your key.' }, { icon: Clock, step: 'Step 3', title: 'Automatic Release', desc: 'Check in on your schedule. If you miss check-ins, we send our piece to your recipient. They combine 2 pieces to reconstruct your key.' }] as item}
			...
			<Card.Root class="bg-muted/50 border-primary/20 mx-auto max-w-2xl">
				<Card.Content class="pt-6">
					...
					<p class="text-muted-foreground text-sm">
						<strong>Zero-knowledge architecture</strong> means we mathematically cannot access your private
						keys. Even if our servers were compromised, your crypto remains secure.
					</p>
				</Card.Content>
			</Card.Root>
		</div>
		...
		<div class="mx-auto max-w-5xl overflow-x-auto">
			<table class="w-full text-left text-sm">
				<thead>
					<tr class="border-b">
						...
						<th class="bg-primary/5 border-primary/20 border-x px-4 py-4 font-semibold">
							<div class="flex items-center gap-2">
								<Shield class="text-primary h-4 w-4" /> KeyFate
							</div>
						</th>
				...
				<tbody class="text-muted-foreground">
					{#each [{ feature: 'Automatic release on death', keyfate: true, paper: false, hw: false, lawyer: 'Slow (probate)' }, { feature: 'Zero-knowledge security', keyfate: true, paper: false, hw: true, lawyer: false }, { feature: 'No single point of failure', keyfate: true, paper: false, hw: false, lawyer: false }, { feature: 'Fire/theft resistant', keyfate: true, paper: false, hw: false, lawyer: true }, { feature: 'No trust required', keyfate: true, paper: true, hw: true, lawyer: false }, { feature: 'Non-technical recipients', keyfate: true, paper: false, hw: false, lawyer: false }] as row}
						<tr class="border-b">
							<td class="px-4 py-3 font-medium">{row.feature}</td>
							<td class="bg-primary/5 border-primary/20 border-x px-4 py-3">
								{#if row.keyfate === true}<CheckCircle
										class="text-primary h-4 w-4"
									/>{:else}<XCircle class="text-destructive h-4 w-4" />{/if}
							</td>
							<td class="px-4 py-3">
								{#if row.paper === true}<CheckCircle class="text-primary h-4 w-4" />{:else}<XCircle
										class="text-destructive h-4 w-4"
									/>{/if}
							</td>
							<td class="px-4 py-3">
								{#if row.hw === true}<CheckCircle class="text-primary h-4 w-4" />{:else}<XCircle
										class="text-destructive h-4 w-4"
									/>{/if}
							</td>
							<td class="px-4 py-3">
								{#if typeof row.lawyer === 'string'}<span class="text-muted-foreground"
										>{row.lawyer}</span
									>{:else if row.lawyer}<CheckCircle class="text-primary h-4 w-4" />{:else}<XCircle
										class="text-destructive h-4 w-4"
									/>{/if}
							</td>
						</tr>
					...
					<tr>
						...
						<td class="bg-primary/5 border-primary/20 text-primary border-x px-4 py-3 font-medium"
	...
	<section class="container mx-auto px-4 py-24">
		...
		<div class="mx-auto grid max-w-4xl gap-6 md:grid-cols-2">
			{#each [{ icon: Shield, title: 'AES-256 Encryption', desc: 'The same encryption standard used by the US government for classified information.' }, { icon: Key, title: "Shamir's Secret Sharing", desc: "Invented by cryptographer Adi Shamir (the 'S' in RSA). Splits your key into pieces where no single piece reveals anything." }, { icon: EyeOff, title: 'Zero-Knowledge Architecture', desc: 'All encryption happens client-side in your browser. We never see, store, or transmit your unencrypted keys.' }, { icon: Lock, title: 'Open-Source Cryptography', desc: 'Our encryption is built on open-source, peer-reviewed algorithms. No security through obscurity.' }] as item}

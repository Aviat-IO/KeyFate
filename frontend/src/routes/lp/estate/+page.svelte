	...
	const digitalAssets = [
		{
			...
			detail: 'Online banking, investment accounts, retirement funds'
		},
		{
			icon: Camera,
			label: 'Photos & videos',
			detail: 'Google Photos, iCloud, family memories spanning decades'
		},
		{
			icon: Mail,
			label: 'Email accounts',
			detail: 'Gmail, Outlook — the keys to resetting everything else'
		},
		{
			icon: Music,
			label: 'Digital purchases',
			detail: 'iTunes, Kindle, Steam — thousands in digital content'
		},
		{
			icon: Cloud,
			label: 'Cloud storage',
			detail: 'Dropbox, Google Drive, OneDrive — important documents'
		},
		{
			icon: Smartphone,
			label: 'Social media accounts',
			detail: 'Facebook, Instagram — years of memories and connections'
		},
		{
			icon: Heart,
			label: 'Health records',
			detail: 'Patient portals, medical history, insurance details'
		},
		{
			icon: Home,
			label: 'Smart home & subscriptions',
			detail: 'Utilities, insurance, streaming — all need passwords'
		}
	];
</script>
...
<div class="bg-background min-h-screen">
		...
		<div class="relative z-10 mx-auto max-w-5xl text-center">
			...
			<p class="text-muted-foreground my-12 text-xl leading-relaxed md:text-2xl">
				...
				<span class="text-foreground font-medium"
					>Passwords, accounts, photos, documents — all inaccessible without you.</span
				>
			</p>
	...
	<section class="bg-muted/30 container mx-auto px-4 py-24">
		<div class="mb-16 text-center">
			...
			<p class="text-muted-foreground mx-auto max-w-2xl text-xl">
				The average person has 100+ online accounts. Here's what your family can't access without
				your passwords.
			</p>
		</div>
			...
			{#each digitalAssets as asset}
				<Card.Root class="group transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
					<Card.Header class="pb-2">
						<asset.icon
							class="text-primary mb-2 h-8 w-8 transition-transform duration-300 group-hover:scale-110"
						/>
			...
			<Card.Root class="border-destructive/20 bg-destructive/5 mx-auto max-w-2xl">
				<Card.Content class="pt-6">
					...
					<p class="text-muted-foreground text-sm">
						When someone dies, tech companies make account recovery incredibly difficult — or
						impossible. Apple, Google, and Facebook all require lengthy legal processes that can
						take months and still fail.
					</p>
				</Card.Content>
			</Card.Root>
		</div>
	...
	<section class="container mx-auto px-4 py-24">
		<div class="mb-16 text-center">
			...
			<p class="text-muted-foreground mx-auto max-w-2xl text-xl">
				Wills were designed for physical assets. Digital lives need a different approach.
			</p>
		</div>

		<div class="mx-auto grid max-w-6xl gap-8 md:grid-cols-3">
			{#each [{ icon: AlertTriangle, title: 'Passwords Change', desc: "A will is static — your digital life isn't", content: "You update passwords regularly. A will written last year already has outdated credentials. KeyFate lets you update secrets anytime — they're always current." }, { icon: Clock, title: 'Probate Takes Months', desc: 'Your family needs access now, not in 6 months', content: 'Wills go through probate — a legal process that takes months or years. Meanwhile, subscriptions charge, bills pile up, and accounts get locked. KeyFate delivers automatically.' }, { icon: EyeOff, title: 'Wills Are Public Record', desc: 'Your passwords would be visible to anyone', content: 'Wills become public documents after probate. Putting passwords in a will means anyone can see them. KeyFate uses zero-knowledge encryption — only your chosen recipient can decrypt.' }] as card}
				<Card.Root class="group transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
					<Card.Header>
						<card.icon
	...
	<section id="how-it-works" class="bg-muted/30 container mx-auto px-4 py-24">
		...
		<div class="mx-auto grid max-w-6xl gap-8 lg:grid-cols-3">
			{#each [{ icon: Lock, step: 'Step 1', title: 'Create & Encrypt', desc: "Enter your passwords, account details, or instructions. Everything is encrypted in your browser using Shamir's Secret Sharing before it ever leaves your device." }, { icon: Shield, step: 'Step 2', title: 'Store Securely', desc: 'Your secret is split into 3 encrypted pieces. We store 1 piece (useless alone). You keep 1. Your recipient gets 1. Nobody can read it individually.' }, { icon: Clock, step: 'Step 3', title: 'Automatic Release', desc: 'Check in on your schedule (weekly, monthly, yearly). If you stop checking in, we automatically deliver our piece to your recipient so they can reconstruct your secrets.' }] as item}
		...
		<div class="mt-16 text-center">
			<Card.Root class="bg-muted/50 border-primary/20 mx-auto max-w-2xl">
				<Card.Content class="pt-6">
					...
					<p class="text-muted-foreground text-sm">
						<strong>We mathematically cannot read your secrets.</strong> All encryption happens in your
						browser. Even if our servers were compromised, your information remains secure.
					</p>
				</Card.Content>
			</Card.Root>
		</div>
	...
	<section class="container mx-auto px-4 py-24">
		<div class="mb-16 text-center">
			...
			<p class="text-muted-foreground mx-auto max-w-2xl text-xl">
				Built on proven, open-source cryptographic standards.
			</p>
		</div>

		<div class="mx-auto grid max-w-4xl gap-6 md:grid-cols-2">
			{#each [{ icon: Shield, title: 'AES-256 Encryption', desc: 'The gold standard of encryption, used by governments and financial institutions worldwide.' }, { icon: Key, title: "Shamir's Secret Sharing", desc: 'A proven cryptographic technique that splits secrets into pieces. No single piece reveals anything.' }, { icon: EyeOff, title: 'Client-Side Only', desc: 'All encryption and decryption happens in your browser. Your plaintext secrets never leave your device.' }, { icon: Lock, title: 'Open-Source Algorithms', desc: 'Our cryptographic methods are based on peer-reviewed, open-source algorithms that anyone can audit.' }] as item}

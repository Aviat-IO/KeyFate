...
import { hasRecentExportRequest, createPendingExportJob } from '$lib/gdpr/export-service';
...
export const POST: RequestHandler = async (event) => {
	try {
		...
		return json({
			jobId: job.id,
			status: job.status,
			message: 'Export request received. You will receive an email when your export is ready.'
		});
	} catch (error) {
		...
		return json({ error: 'Failed to create export request' }, { status: 500 });
	}
};

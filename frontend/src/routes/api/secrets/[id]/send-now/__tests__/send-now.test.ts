...
vi.mock('$lib/email/email-service', () => ({
	sendSecretDisclosureEmail: (...args: unknown[]) => mockSendSecretDisclosureEmail(...args)
}));
...
function createEvent(secretId = TEST_SECRET_ID) {
	return {
		...
		request: new Request(`http://localhost/api/secrets/${secretId}/send-now`, { method: 'POST' })
	} as any;
}
...
describe('POST /api/secrets/[id]/send-now', () => {
	...
	describe('Status validation', () => {
		it("returns 400 if secret status is not 'failed'", async () => {
			setupDbSequence([{ type: 'select', result: [{ ...BASE_SECRET, status: 'active' }] }]);
		...
		it('returns 400 for triggered status', async () => {
			setupDbSequence([{ type: 'select', result: [{ ...BASE_SECRET, status: 'triggered' }] }]);
	...
	describe('Disclosure check', () => {
		it('returns 400 if secret has already been disclosed', async () => {
			...
			expect(response.status).toBe(400);
			const data = await response.json();
			expect(data.error).toBe('This secret has already been disclosed to recipients');
	...
	describe('Successful send', () => {
		...
		it('sends to multiple recipients successfully', async () => {
			...
			let insertIdx = 0;
			const insertResults = [[{ id: 'log-1' }], [{ id: 'log-2' }]];
	...
	describe('Partial failure', () => {
		...
		it('handles email send throwing an exception', async () => {
			...
			mockGetAllRecipients.mockResolvedValue([BASE_RECIPIENT]);
			mockSendSecretDisclosureEmail.mockRejectedValue(new Error('Network timeout'));

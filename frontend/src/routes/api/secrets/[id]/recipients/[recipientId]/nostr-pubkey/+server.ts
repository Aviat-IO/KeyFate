 ...
 */
async function getOwnedRecipient(userId: string, secretId: string, recipientId: string) {
	...
	const [recipient] = await db
		...
		.where(and(eq(secretRecipients.id, recipientId), eq(secretRecipients.secretId, secretId)));
...
export const PUT: RequestHandler = async (event) => {
	try {
		...
		const recipient = await getOwnedRecipient(session.user.id, secretId, recipientId);
		...
		if (err instanceof z.ZodError) {
			return json({ error: 'Invalid data', details: err.issues }, { status: 400 });
		}

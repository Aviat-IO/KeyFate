...
export const PUT: RequestHandler = async (event) => {
	try {
		...
		const invalidRecipients = validatedData.recipients.filter((r) => !r.email && !r.phone);
		...
		const checkInDaysChanged = existingSecret.checkInDays !== validatedData.check_in_days;

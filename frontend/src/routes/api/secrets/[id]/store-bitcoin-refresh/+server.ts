...
export const POST: RequestHandler = async (event) => {
	try {
		...
		if (typeof body.newTxId !== 'string' || !/^[0-9a-f]{64}$/i.test(body.newTxId)) {
			return json({ error: 'newTxId must be a 64-character hex string' }, { status: 400 });
		}
		if (typeof body.newOutputIndex !== 'number' || body.newOutputIndex < 0) {
			return json({ error: 'newOutputIndex must be a non-negative integer' }, { status: 400 });
		}
		if (typeof body.newAmountSats !== 'number' || body.newAmountSats < 1) {
			return json({ error: 'newAmountSats must be a positive integer' }, { status: 400 });
		}
		if (typeof body.newTimelockScript !== 'string') {
			return json({ error: 'newTimelockScript must be a hex string' }, { status: 400 });
		}
		...
		if (!currentUtxo) {
			return json({ error: 'No active Bitcoin UTXO found for this secret' }, { status: 404 });
		}

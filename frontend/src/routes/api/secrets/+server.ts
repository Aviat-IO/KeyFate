...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		const emailVerificationError = await requireEmailVerification(
			session as Parameters<typeof requireEmailVerification>[0]
		);
			...
			const insertedRecipients = await tx
				...
				.values(
					validatedData.recipients.map((recipient) => ({
						...
						nostrPubkey: recipient.email ? (nostrPubkeyByEmail.get(recipient.email) ?? null) : null
					}))
				)
				.returning();

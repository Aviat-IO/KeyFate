...
import { parseOffsetPagination, buildOffsetPaginatedResponse } from '$lib/api/pagination';
...
export const GET: RequestHandler = async (event) => {
	try {
		...
		if (eventType) {
			conditions.push(
				eq(auditLogs.eventType, eventType as (typeof auditLogs.eventType.enumValues)[number])
			);
		}
		...
		if (eventCategory) {
			conditions.push(
				eq(
					...
					eventCategory as (typeof auditLogs.eventCategory.enumValues)[number]
				)
			);
		}

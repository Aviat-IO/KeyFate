...
export function createCursorPaginationMeta<T extends { id: string }>(
	...
	const nextCursor = hasMore && dataItems.length > 0 ? dataItems[dataItems.length - 1].id : null;
...
export function buildPaginationLinks(
	...
	return {
		...
		prev: params.offset > 0 ? buildUrl(Math.max(0, params.offset - params.limit)) : null
	};
}

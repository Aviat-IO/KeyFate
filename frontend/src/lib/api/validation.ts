 ...
 * Validation result
 */
export type ValidationResult<T> = { success: true; data: T } | { success: false; error: APIError };
...
export function validateSearchParams<T extends ZodSchema>(
	...
	try {
		...
		return {
			success: false,
			error: APIError.internal('Failed to validate query parameters', error as Error)
		};
	}
}
...
export function validateParams<T extends ZodSchema>(
	...
	try {
		...
		return {
			success: false,
			error: APIError.internal('Failed to validate path parameters', error as Error)
		};
	}
}
 ...
 */
export function asyncRefinement<T>(validator: (value: T) => Promise<boolean>, message: string) {

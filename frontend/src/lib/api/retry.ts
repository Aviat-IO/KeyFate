 ...
 * ```
 */
export async function retry<T>(fn: () => Promise<T>, userConfig: RetryConfig = {}): Promise<T> {
...
export async function retryBatch<T>(
	...
	const results = await Promise.all(operations.map((op) => retryWithResult(op, config)));

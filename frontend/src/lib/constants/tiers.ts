import type { SubscriptionTier, TierConfig } from '$lib/types/subscription';
...
export const TIER_CONFIGS: Partial<Record<SubscriptionTier, TierConfig>> = {
	...
	pro: {
		...
		id: 'pro',
		displayName: 'Pro',
		description: 'For power users who need maximum capacity and premium features',
		maxSecrets: 10,
...
export function getLookupKey(tier: SubscriptionTier, period: 'monthly' | 'annual'): string | null {
...
export function getPriceInCents(tier: SubscriptionTier, period: 'monthly' | 'annual'): number {
	const config = getTierConfig(tier);

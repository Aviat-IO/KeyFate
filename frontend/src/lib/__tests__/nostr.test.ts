...
describe('Nostr Keypair', () => {
	...
	it('derives the correct public key from a secret key', async () => {
		const { generateKeypair, publicKeyFromSecret } = await import('$lib/nostr/keypair');
		const kp = generateKeypair();
		expect(publicKeyFromSecret(kp.secretKey)).toBe(kp.publicKey);
	});

	it('round-trips npub encoding', async () => {
		const { generateKeypair, npubToHex, hexToNpub } = await import('$lib/nostr/keypair');
		const kp = generateKeypair();
	...
	describe('isValidNpub', () => {
		it('returns true for a valid npub', async () => {
			const { isValidNpub, generateKeypair } = await import('$lib/nostr/keypair');
			const kp = generateKeypair();
	...
	describe('isValidHexPubkey', () => {
		it('returns true for a 64-char hex string', async () => {
			const { isValidHexPubkey, generateKeypair } = await import('$lib/nostr/keypair');
			const kp = generateKeypair();
...
describe('Nostr NIP-44 Encryption', () => {
	it('encrypts and decrypts a message between two keypairs', async () => {
		const { generateKeypair } = await import('$lib/nostr/keypair');
		const { getConversationKey, encrypt, decrypt } = await import('$lib/nostr/encryption');
		...
		const recipientConvKey = getConversationKey(recipient.secretKey, sender.publicKey);
	...
	it('produces different ciphertext for the same plaintext', async () => {
		const { generateKeypair } = await import('$lib/nostr/keypair');
		const { getConversationKey, encrypt } = await import('$lib/nostr/encryption');
	...
	it('decryption fails with wrong key', async () => {
		...
		const wrongKey = getConversationKey(wrongRecipient.secretKey, sender.publicKey);
...
describe('Nostr NIP-59 Gift Wrap', () => {
	it('createRumor produces an unsigned event with correct kind', async () => {
		const { generateKeypair } = await import('$lib/nostr/keypair');
		const { createRumor, KEYFATE_SHARE_KIND } = await import('$lib/nostr/gift-wrap');
	...
	it('createGiftWrap produces a kind 1059 event with p-tag', async () => {
		const { generateKeypair } = await import('$lib/nostr/keypair');
		const { createRumor, createSeal, createGiftWrap } = await import('$lib/nostr/gift-wrap');
	...
	it('recipient can unwrap the gift wrap to recover the share payload', async () => {
		const { generateKeypair } = await import('$lib/nostr/keypair');
		const { wrapShareForRecipient } = await import('$lib/nostr/gift-wrap');
		const { getConversationKey, decrypt } = await import('$lib/nostr/encryption');
		...
		const wrap = wrapShareForRecipient(originalPayload, sender.secretKey, recipient.publicKey);
...
describe('Nostr Client', () => {
	...
	it('createNostrClient factory returns a NostrClient', async () => {
		const { createNostrClient, NostrClient } = await import('$lib/nostr/client');

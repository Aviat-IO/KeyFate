...
describe('Bitcoin CSV Timelock - Script', () => {
	...
	describe('createCSVTimelockScript', () => {
		it('creates a valid script with correct structure', async () => {
			const { createCSVTimelockScript, decodeCSVTimelockScript } =
				await import('$lib/bitcoin/script');
			const script = createCSVTimelockScript(owner.pubkey, recipient.pubkey, 144);
		...
		it('handles maximum CSV value (65535 blocks)', async () => {
			const { createCSVTimelockScript, decodeCSVTimelockScript } =
				await import('$lib/bitcoin/script');
			const script = createCSVTimelockScript(owner.pubkey, recipient.pubkey, 65535);
			const decoded = decodeCSVTimelockScript(script);
		...
		it('handles minimum CSV value (1 block)', async () => {
			const { createCSVTimelockScript, decodeCSVTimelockScript } =
				await import('$lib/bitcoin/script');
			const script = createCSVTimelockScript(owner.pubkey, recipient.pubkey, 1);
			const decoded = decodeCSVTimelockScript(script);
		...
		it('throws for invalid owner pubkey length', async () => {
			const { createCSVTimelockScript } = await import('$lib/bitcoin/script');
			expect(() => createCSVTimelockScript(new Uint8Array(32), recipient.pubkey, 144)).toThrow(
				'Owner pubkey must be 33 bytes'
			);
		});

		it('throws for invalid recipient pubkey length', async () => {
			const { createCSVTimelockScript } = await import('$lib/bitcoin/script');
			expect(() => createCSVTimelockScript(owner.pubkey, new Uint8Array(65), 144)).toThrow(
				'Recipient pubkey must be 33 bytes'
			);
		});

		it('throws for ttlBlocks = 0', async () => {
			const { createCSVTimelockScript } = await import('$lib/bitcoin/script');
			expect(() => createCSVTimelockScript(owner.pubkey, recipient.pubkey, 0)).toThrow(
				'ttlBlocks must be between 1 and 65535'
			);
		});

		it('throws for ttlBlocks > MAX_CSV_BLOCKS', async () => {
			const { createCSVTimelockScript } = await import('$lib/bitcoin/script');
			expect(() => createCSVTimelockScript(owner.pubkey, recipient.pubkey, 65536)).toThrow(
				'ttlBlocks must be between 1 and 65535'
			);
		});
		...
		it('throws for non-integer ttlBlocks', async () => {
			const { createCSVTimelockScript } = await import('$lib/bitcoin/script');
			expect(() => createCSVTimelockScript(owner.pubkey, recipient.pubkey, 144.5)).toThrow(
				'ttlBlocks must be an integer'
			);
		});

		it('produces deterministic output for same inputs', async () => {
			const { createCSVTimelockScript } = await import('$lib/bitcoin/script');
			const script1 = createCSVTimelockScript(owner.pubkey, recipient.pubkey, 144);
			const script2 = createCSVTimelockScript(owner.pubkey, recipient.pubkey, 144);
		...
		it('produces different output for different TTLs', async () => {
			const { createCSVTimelockScript } = await import('$lib/bitcoin/script');
			const script1 = createCSVTimelockScript(owner.pubkey, recipient.pubkey, 144);
			const script2 = createCSVTimelockScript(owner.pubkey, recipient.pubkey, 288);
	...
	describe('createP2WSHAddress', () => {
		it('generates a valid mainnet bech32 address', async () => {
			const { createCSVTimelockScript, createP2WSHAddress } = await import('$lib/bitcoin/script');
			const script = createCSVTimelockScript(owner.pubkey, recipient.pubkey, 144);
	...
	describe('decodeCSVTimelockScript', () => {
		it('round-trips with createCSVTimelockScript', async () => {
			...
			const script = createCSVTimelockScript(owner.pubkey, recipient.pubkey, ttlBlocks);
			const decoded = decodeCSVTimelockScript(script);
		...
		it('throws for invalid script', async () => {
			...
			expect(() => decodeCSVTimelockScript(badScript)).toThrow('Invalid CSV timelock script');
		});
	});
	...
	describe('createOpReturnPayload', () => {
		...
		it('throws for wrong key length', async () => {
			const { createOpReturnPayload } = await import('$lib/bitcoin/script');
			expect(() => createOpReturnPayload(new Uint8Array(16), 'ab'.repeat(32))).toThrow(
		...
		it('throws for wrong event ID length', async () => {
			const { createOpReturnPayload } = await import('$lib/bitcoin/script');
			expect(() => createOpReturnPayload(new Uint8Array(32), 'ab'.repeat(16))).toThrow(
				'Nostr event ID must be 64 hex chars'
			);
		});
	});
	...
	describe('createOpReturnScript', () => {
		...
		it('throws for data > 80 bytes', async () => {
			const { createOpReturnScript } = await import('$lib/bitcoin/script');
			expect(() => createOpReturnScript(new Uint8Array(81))).toThrow('OP_RETURN data too large');
		});
...
describe('Bitcoin CSV Timelock - Transaction', () => {
	...
	describe('createPreSignedRecipientTx', () => {
		it('creates a valid pre-signed recipient transaction', async () => {
			const { createTimelockUTXO, createPreSignedRecipientTx } =
				await import('$lib/bitcoin/transaction');
			const p2wpkh = btc.p2wpkh(owner.pubkey);
		...
		it('throws when UTXO too small for fees', async () => {
			const { createPreSignedRecipientTx } = await import('$lib/bitcoin/transaction');
			const { createCSVTimelockScript } = await import('$lib/bitcoin/script');
	...
	describe('estimateTimelockCreationVbytes', () => {
		it('returns a reasonable estimate', async () => {
			const { estimateTimelockCreationVbytes } = await import('$lib/bitcoin/transaction');
	...
	describe('estimateRecipientSpendVbytes', () => {
		it('returns a reasonable estimate', async () => {
			const { estimateRecipientSpendVbytes } = await import('$lib/bitcoin/transaction');
...
describe('Bitcoin CSV Timelock - Refresh', () => {
	...
	describe('estimateRefreshesRemaining', () => {
		it('estimates correct number of refreshes', async () => {
			const { estimateRefreshesRemaining } = await import('$lib/bitcoin/refresh');
...
describe('Bitcoin CSV Timelock - Broadcast', () => {
	...
	describe('broadcastTransaction', () => {
		...
		it('throws when all endpoints fail', async () => {
			...
			await expect(broadcastTransaction('deadbeef', 'mainnet')).rejects.toThrow(
				'Failed to broadcast'
			);
...
describe('Bitcoin CSV Timelock - Integration', () => {
	it('full lifecycle: create → pre-sign → refresh', async () => {
		const { createTimelockUTXO, createPreSignedRecipientTx } =
			await import('$lib/bitcoin/transaction');
		const { refreshTimelockUTXO } = await import('$lib/bitcoin/refresh');
		const { decodeCSVTimelockScript, createP2WSHAddress } = await import('$lib/bitcoin/script');
	...
	it('multiple sequential refreshes', async () => {
		const { refreshTimelockUTXO, estimateRefreshesRemaining } =
			await import('$lib/bitcoin/refresh');
		const { createCSVTimelockScript } = await import('$lib/bitcoin/script');

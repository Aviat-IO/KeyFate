...
import { recoverKFromOpReturn, recoverKFromPassphrase, decryptShare } from '$lib/crypto/recovery';
...
import { createTimelockUTXO, createPreSignedRecipientTx } from '$lib/bitcoin/transaction';
...
describe('E2E 7.3: Create secret with Bitcoin -> timeout -> recipient recovers', () => {
	it('full flow: encrypt share, embed K in OP_RETURN, recipient recovers', async () => {
		...
		const timelockScript = createCSVTimelockScript(ownerBtc.pubkey, recipientBtc.pubkey, ttlBlocks);
		...
		expect(hex.encode(decoded.recipientPubkey)).toBe(hex.encode(recipientBtc.pubkey));
	...
	it('OP_RETURN K in pre-signed tx matches the encryption key', async () => {
		...
		// Decrypt the share
		const recovered = decryptShare(encrypted.encryptedShare, encrypted.nonce, extractedK);
...
describe('E2E 7.5: Quantum-safe recovery (passphrase + OP_RETURN K, no NIP-44)', () => {
	it('recovers share using only passphrase — no secp256k1/NIP-44 involved', async () => {
		// 1. Generate a share
		const originalShare = 'quantum-safe-share:deadbeef0123456789abcdef';

		// 2. Encrypt with ChaCha20 using random K
		const K = generateSymmetricKey();
		const shareBytes = new TextEncoder().encode(originalShare);
		const { ciphertext: encryptedShare, nonce: shareNonce } = encryptWithSymmetricKey(
			shareBytes,
			K
		);
		...
		const { ciphertext: encryptedK, nonce: kNonce } = await encryptWithDerivedKey(K, derivedKey);
		...
		const kFromPassphrase = await recoverKFromPassphrase(passphraseBundle, passphrase);
		...
		const shareFromPassphrase = decryptShare(encryptedShare, shareNonce, kFromPassphrase);
		expect(shareFromPassphrase).toBe(originalShare);

		const shareFromOpReturn = decryptShare(encryptedShare, shareNonce, kFromOpReturn);
	...
	it('passphrase recovery fails with wrong passphrase', async () => {
		...
		await expect(
			recoverKFromPassphrase({ ciphertext, nonce, salt }, 'wrong-passphrase')
		).rejects.toThrow();
	});
	...
	it('full quantum-safe pipeline: encrypt share, encrypt K two ways, recover both', async () => {
		...
		const opReturnPayload = createOpReturnPayload(K, 'dd'.repeat(32));

		// Recovery path 1: passphrase
		const k1 = await recoverKFromPassphrase({ ciphertext: encK, nonce: kNonce, salt }, passphrase);
	...
	it('AES-256-GCM detects tampering on passphrase-encrypted K', async () => {
		...
		await expect(
			recoverKFromPassphrase({ ciphertext: tampered, nonce, salt }, passphrase)
		).rejects.toThrow();
	});
});

...
describe('Encryption Functions', () => {
	...
	describe('decryptMessage', () => {
		it('should decrypt a message using cipher text, IV, and auth tag', async () => {
			const { encryptMessage, decryptMessage } = await import('$lib/encryption');
			...
			const result = await decryptMessage(encrypted.encrypted, ivBuffer, authTagBuffer);
		...
		it('should handle decryption errors gracefully', async () => {
			...
			await expect(decryptMessage(invalidCipherText, ivBuffer, authTag)).rejects.toThrow();
		});
	});
	...
	describe('Encryption/Decryption Round Trip', () => {
		it('should successfully encrypt and decrypt a message', async () => {
			...
			const ivBuffer = Buffer.from(encrypted.iv, 'base64');
			const authTagBuffer = Buffer.from(encrypted.authTag, 'base64');
			const decrypted = await decryptMessage(encrypted.encrypted, ivBuffer, authTagBuffer);

...
describe('symmetric (ChaCha20-Poly1305)', () => {
	...
	describe('encryptWithSymmetricKey', () => {
		...
		it('throws on invalid key length', () => {
			...
			expect(() => encryptWithSymmetricKey(plaintext, badKey)).toThrow('Key must be 32 bytes');
		});
	...
	describe('decryptWithSymmetricKey', () => {
		...
		it('throws on wrong nonce', () => {
			...
			expect(() => decryptWithSymmetricKey(ciphertext, wrongNonce, key)).toThrow();
		});

		it('throws on invalid key length', () => {
			expect(() =>
				decryptWithSymmetricKey(new Uint8Array(32), new Uint8Array(12), new Uint8Array(16))
			).toThrow('Key must be 32 bytes');
		});

		it('throws on invalid nonce length', () => {
			expect(() =>
				decryptWithSymmetricKey(new Uint8Array(32), new Uint8Array(8), new Uint8Array(32))
...
describe('passphrase (PBKDF2 + AES-256-GCM)', () => {
	describe('deriveKeyFromPassphrase', () => {
		...
		it('derives same key with same salt', async () => {
			...
			const ct1 = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: nonce }, r1.key, data);
			// Decrypt with the other key
			const pt = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: nonce }, r2.key, ct1);
			expectBytesEqual(new Uint8Array(pt), data);
		});

		it('throws on empty passphrase', async () => {
			await expect(deriveKeyFromPassphrase('')).rejects.toThrow('Passphrase must not be empty');
		});
	});
	...
	describe('encryptWithDerivedKey / decryptWithDerivedKey', () => {
		...
		it('fails decryption with wrong key', async () => {
			...
			const { ciphertext, nonce } = await encryptWithDerivedKey(new Uint8Array([1, 2, 3]), key1);

			await expect(decryptWithDerivedKey(ciphertext, nonce, key2)).rejects.toThrow();
		});
	});
});
...
describe('doubleEncryptShare', () => {
	...
	it('encrypted share can be decrypted with plaintextK', async () => {
		...
		const result = await doubleEncryptShare(share, fakePubkey, fakePrivkey, undefined, testNip44);
	...
	it('encryptedKNostr can be decrypted to recover K', async () => {
		...
		const kHex = testNip44.decrypt(result.encryptedKNostr, fakePrivkey, fakePubkey) as string;
	...
	it('generates unique K for each call', async () => {
		const r1 = await doubleEncryptShare('share', fakePubkey, fakePrivkey, undefined, testNip44);
		const r2 = await doubleEncryptShare('share', fakePubkey, fakePrivkey, undefined, testNip44);
...
describe('recovery', () => {
	...
	describe('recoverKFromNostr', () => {
		it('recovers K from NIP-44 encrypted payload', async () => {
			...
			const recovered = await recoverKFromNostr(encrypted, fakePrivkey, fakePubkey, testNip44);
	...
	describe('recoverKFromPassphrase', () => {
		it('recovers K from passphrase-encrypted payload', async () => {
			...
			const recovered = await recoverKFromPassphrase({ ciphertext, nonce, salt }, passphrase);
...
describe('end-to-end double encryption', () => {
	...
	it('encrypts and recovers share via passphrase path', async () => {
		...
		const K = await recoverKFromPassphrase(encrypted.encryptedKPassphrase!, passphrase);
	...
	it('all three recovery paths yield the same K', async () => {
		...
		const kPassphrase = await recoverKFromPassphrase(encrypted.encryptedKPassphrase!, passphrase);
		...
		expect(decryptShare(encrypted.encryptedShare, encrypted.nonce, kNostr)).toBe(share);
		expect(decryptShare(encrypted.encryptedShare, encrypted.nonce, kPassphrase)).toBe(share);
		expect(decryptShare(encrypted.encryptedShare, encrypted.nonce, kOpReturn)).toBe(share);
	});
	...
	it('handles large shares', async () => {
		...
		const K = recoverKFromOpReturn(encrypted.plaintextK);
		expect(decryptShare(encrypted.encryptedShare, encrypted.nonce, K)).toBe(share);
	});
});

...
import type { SubscriptionTier, TierLimits, UserTierInfo } from '$lib/types/subscription';
...
export async function calculateUserUsage(userId: string) {
	try {
		...
		const [result] = await db
			...
			.from(secrets)
			.where(and(eq(secrets.userId, userId), inArray(secrets.status, ['active', 'paused'])));
...
export function isIntervalAllowed(tier: SubscriptionTier, intervalDays: number): boolean {
	const config = getTierConfig(tier);
	if (!config) {
		return false;
	}

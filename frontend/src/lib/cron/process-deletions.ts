...
import { getPendingDeletions, executeAccountDeletion } from '$lib/gdpr/deletion-service';
...
export async function runProcessDeletions(): Promise<ProcessDeletionsResult> {
	...
	for (const deletion of pendingDeletions) {
		try {
			...
			logger.error('Error processing deletion', error instanceof Error ? error : undefined, {

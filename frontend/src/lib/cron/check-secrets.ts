...
import { CRON_CONFIG, isApproachingTimeout, logCronMetrics, sanitizeError } from '$lib/cron/utils';
...
async function markReminderSent(db: DatabaseConnection, reminderId: string): Promise<void> {
	try {
		const now = new Date();
		...
		await db
			...
			.where(
				and(
					...
					lt(sql`COALESCE(${reminderJobs.retryCount}, 0)`, CRON_CONFIG.MAX_RETRIES)
				)
			);
...
function calculateUrgency(nextCheckIn: Date): 'critical' | 'high' | 'medium' | 'low' {
	const now = new Date();
	...
	try {
		...
		if (!result.success) {
			...
			const [failureStats] = await db
				...
				.from(emailFailures)
				.where(
					and(eq(emailFailures.emailType, 'reminder'), eq(emailFailures.recipient, user.email))
				)
			...
			if (reminderId) {
				await markReminderFailed(db, reminderId, result.error || 'Unknown error');
			}
...
async function handleMissedReminders(db: DatabaseConnection): Promise<{ marked: number }> {
	const now = new Date();
			...
			if (remindersProcessed === 0 && retryStats.processed === 0) {
				try {
					...
					if (Number(stuckCount?.count ?? 0) > 0) {
						logger.warn('Reminders have exhausted all retries and will not be processed', {

...
export class CronMonitor {
	...
	getJobStats(jobName: string): CronJobStats {
		...
		return {
			...
			lastSuccess:
				successJobs.length > 0 ? Math.max(...successJobs.map((j) => j.endTime || 0)) : null,
			lastFailure:
				failureJobs.length > 0 ? Math.max(...failureJobs.map((j) => j.endTime || 0)) : null
		};
	}
	 ...
	 */
	private detectAnomalies(jobName: string, currentJob: CronJobMetrics, stats: CronJobStats): void {
		...
		const recentJobs = (this.metrics.get(jobName) || [])
			...
			.slice(-5);
		const consecutiveFailures = recentJobs.reverse().findIndex((j) => j.status === 'success');
		...
		const resultMetadata =
			typeof result === 'object' && result !== null
				? {
						...
						succeeded: ((result as Record<string, unknown>).succeeded ||
							(result as Record<string, unknown>).sent) as number | undefined,

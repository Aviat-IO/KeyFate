...
export async function initiateAccountDeletion(userId: string) {
	...
	const scheduledDeletionAt = new Date(Date.now() + GRACE_PERIOD_DAYS * 24 * 60 * 60 * 1000);
...
export async function cancelAccountDeletion(requestId: string, userId: string) {
	...
	const [request] = await db
		...
		.from(accountDeletionRequests)
		.where(
			and(eq(accountDeletionRequests.id, requestId), eq(accountDeletionRequests.userId, userId))
		);
...
export async function executeAccountDeletion(userId: string) {
	...
	try {
		...
		if (userEmail) {
			await db.delete(emailFailures).where(eq(emailFailures.recipient, userEmail));
		}
		...
		if (secretIds.length > 0) {
			for (const secretId of secretIds) {
				await db.delete(secretRecipients).where(eq(secretRecipients.secretId, secretId));
			}
		}
		...
		if (userEmail) {
			await db.delete(verificationTokens).where(eq(verificationTokens.identifier, userEmail));
		}
...
export async function getActiveDeletionRequest(userId: string) {
	...
	if (request && (request.status === 'pending' || request.status === 'confirmed')) {
		return request;
	}

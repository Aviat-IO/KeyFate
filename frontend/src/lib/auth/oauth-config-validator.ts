...
export function validateOAuthConfig(config?: OAuthConfig): ValidationResult {
	...
	if (!authSecret) {
		...
		warnings.push('AUTH_SECRET should be at least 32 characters long for security');
	}

	if (!siteUrl) {
		errors.push('PUBLIC_SITE_URL is required for Auth.js');
	} else if (!siteUrl.match(/^https?:\/\//)) {
		errors.push('PUBLIC_SITE_URL must be a valid URL starting with http:// or https://');
	}
	...
	const isBuildTime = process.env.NODE_ENV === 'production' && siteUrl?.includes('localhost');
	...
	if (process.env.NODE_ENV === 'production' && !isBuildTime) {
		...
		if (authSecret === 'your-nextauth-secret-here') {
			errors.push('AUTH_SECRET must be changed from default value in production');
		}
	}

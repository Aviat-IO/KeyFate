...
export class APIError extends Error {
	...
	static rateLimit(message = 'Too many requests', retryAfter?: number): APIError {
	...
	static businessRule(message: string, details?: Record<string, unknown>): APIError {
	...
	static externalService(service: string, message?: string, cause?: Error): APIError {
		const errorMessage = message ?? `External service error: ${service} is unavailable`;
	...
	static timeout(operation: string): APIError {
		return new APIError(`Operation timed out: ${operation}`, ErrorCode.TIMEOUT, 504, {
			severity: ErrorSeverity.HIGH,
			details: { operation },
			isOperational: true
		});

...
class ConnectionManager {
	...
	private setupGracefulShutdown(): void {
		const shutdown = async (signal: string) => {
			...
			try {
				console.log(`⏳ Waiting for ${this.activeQueries} active queries to complete...`);
				...
				if (this.activeQueries > 0) {
					console.warn(`⚠️ ${this.activeQueries} queries still active after ${maxWait}ms`);
				}
	...
	async getConnection(
		...
		const isUnixSocket =
			connectionString.includes('/cloudsql/') || connectionString.includes('host=/cloudsql/');
		const isPrivateIP =

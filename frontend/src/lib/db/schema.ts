...
export const disclosureStatusEnum = pgEnum('disclosure_status', ['pending', 'sent', 'failed']);
...
export const exportJobStatusEnum = pgEnum('export_job_status', enumToPgEnum(ExportJobStatus));
...
export const otpRateLimits = pgTable(
	...
	(table) => ({
		emailWindowIdx: index('idx_otp_rate_limits_email_window').on(table.email, table.windowEnd)
	})
);
...
export const accountLockouts = pgTable(
	...
	(table) => ({
		...
		lockedUntilIdx: index('idx_account_lockouts_locked_until').on(table.lockedUntil),
...
export const privacyPolicyAcceptance = pgTable(
	'privacy_policy_acceptance',
	{
		...
		policyDocumentId: uuid('policy_document_id').references(() => policyDocuments.id), // Link to exact policy content
		termsDocumentId: uuid('terms_document_id').references(() => policyDocuments.id), // Link to exact terms content
...
export const secrets = pgTable(
	...
	(table) => ({
		userStatusIdx: index('secrets_user_status_idx').on(table.userId, table.status),
...
export const reminderJobs = pgTable(
	...
	(table) => ({
		secretReminderStatusIdx: index('reminder_jobs_secret_reminder_status_idx').on(
			table.secretId,
			table.reminderType,
			table.status,
			table.sentAt
...
export const disclosureLog = pgTable(
	...
	(table) => ({
		...
		secretRecipientDisclosedIdx: index('idx_disclosure_log_secret_recipient').on(
			table.secretId,
			table.recipientEmail,
			table.sentAt
		)
...
export const accountDeletionRequests = pgTable(
	'account_deletion_requests',
	{
		...
		userId: text('user_id')
			.notNull()
			.references(() => users.id, { onDelete: 'cascade' }),
		status: deletionRequestStatusEnum('status').notNull().default(DeletionRequestStatus.PENDING),
		confirmationToken: text('confirmation_token').notNull().unique(),
		confirmationSentAt: timestamp('confirmation_sent_at', { mode: 'date' }).defaultNow().notNull(),
	...
	(table) => ({
		userStatusIdx: index('idx_deletion_requests_user_status').on(table.userId, table.status),

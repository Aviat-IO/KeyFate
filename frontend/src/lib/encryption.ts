...
function getEncryptionKey(version?: number): Buffer {
	...
	if (!ENCRYPTION_KEYS.has(targetVersion)) {
		...
		const ENCRYPTION_KEY_BASE64 =
			process.env[keyEnvVar] || (targetVersion === 1 ? serverEnv.ENCRYPTION_KEY : null);
...
export async function encryptMessage(
	...
	const cipher = crypto.createCipheriv(ALGORITHM, getEncryptionKey(version), ivBuffer);
...
export async function decryptMessage(
	...
	const decipher = crypto.createDecipheriv(ALGORITHM, getEncryptionKey(version), ivBuffer);

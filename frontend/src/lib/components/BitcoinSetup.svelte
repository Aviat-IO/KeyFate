<script lang="ts">
	...
	async function handleEnableBitcoin() {
		...
		if (PLACEHOLDER_SYMMETRIC_KEY.every((b) => b === 0)) {
			errorMessage =
				'Symmetric key not available — cannot create Bitcoin timelock. Nostr integration must be completed first.';
			return;
		}
...
<Card.Root>
				...
				<Select.Root type="single" bind:value={feePriority}>
					...
					<Select.Content>
						<Select.Item value="low"
							>{feeEstimates.low.label} (~{feeEstimates.low.rate} sat/vB)</Select.Item
						>
						<Select.Item value="medium"
							>{feeEstimates.medium.label} (~{feeEstimates.medium.rate} sat/vB)</Select.Item
						>
						<Select.Item value="high"
							>{feeEstimates.high.label} (~{feeEstimates.high.rate} sat/vB)</Select.Item
						>
					</Select.Content>
				</Select.Root>
			</div>
			...
			<div class="space-y-4">
				<div class="space-y-2">
					...
					<div class="bg-muted flex items-center gap-2 rounded-lg p-3">
						<code class="flex-1 font-mono text-xs break-all">{fundingAddress}</code>
		...
		{:else if step === 'processing'}
			<div class="flex flex-col items-center gap-3 py-6">
				...
				<p class="text-muted-foreground text-sm">
					Creating and broadcasting timelock transaction...
				</p>
		...
		{:else if step === 'done'}
			<div class="flex flex-col items-center gap-3 py-6">
				...
				<p class="text-muted-foreground text-center text-xs">
					Your funds are locked. The recipient can claim them after {ttlBlocks} blocks (~{checkInDays}
					days) if you stop checking in.
				</p>
			</div>
		{/if}
	</Card.Content>
	...
	<Card.Footer class="flex gap-2">
		...
		{:else if step === 'fund'}
			<Button variant="outline" onclick={handleStartOver} class="shrink-0">Back</Button>
			<Button
		...
		{:else if step === 'confirm'}
			<Button variant="outline" onclick={() => (step = 'fund')} class="shrink-0">Back</Button>

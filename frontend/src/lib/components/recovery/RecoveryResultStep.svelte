<script lang="ts">
	...
	import { Shield, CheckCircle, AlertTriangle, Copy, ArrowLeft } from '@lucide/svelte';
...
<div class="space-y-6">
	...
	{#each decryptedShares as result, i}
		<div class="bg-muted rounded-lg p-4">
			<div class="mb-2 flex items-center justify-between">
				...
				<Button variant="ghost" size="sm" onclick={() => copyToClipboard(result.share)}>
					<Copy class="mr-1 h-3 w-3" />
					Copy
				</Button>
			</div>
			<pre
				class="bg-background overflow-x-auto rounded border p-3 font-mono text-xs break-all whitespace-pre-wrap">{result.share}</pre>
	...
	<div class="space-y-4">
		...
		<p class="text-muted-foreground text-sm">
			{recoveredShares.length} share{recoveredShares.length !== 1 ? 's' : ''} recovered so far. You need
			at least 2 shares to reconstruct the original secret.
		</p>

		{#if !showReconstruct}
			<div class="flex flex-col gap-3 sm:flex-row">
				<Button variant="outline" onclick={onRecoverAnother} class="flex-1">
			...
			{/if}
		{:else}
			<Button variant="ghost" onclick={() => (showReconstruct = false)} class="mb-2">
				<ArrowLeft class="mr-2 h-4 w-4" />
				Back to shares
			</Button>

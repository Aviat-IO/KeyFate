...
<div class="space-y-6">
	...
	<div class="space-y-2">
		...
		<Alert.Alert variant="destructive" class="mb-2">
			...
			<Alert.AlertDescription class="text-xs">
				Your nsec is your Nostr identity. Never share it. It is only used locally to decrypt events
		...
		<div class="relative">
			...
			<Button
				variant="ghost"
				size="sm"
				class="absolute top-1 right-1 h-8 w-8 p-0"
			...
			<div class="space-y-2">
				{#each foundEvents as gw, i}
					<button
						...
						class="border-border hover:bg-accent w-full rounded-lg border p-3 text-left transition-colors {selectedEventIndices.has(
							i
						)
					...
					>
						<div class="flex items-center justify-between">
							...
							<div
								class="flex h-5 w-5 items-center justify-center rounded border {selectedEventIndices.has(
									i
								)
	...
	{#if unwrappedShares.length > 0}
		...
		<div class="space-y-4">
			...
			{#if kRecoveryMethod}
				<Button onclick={decryptSharesWithK} disabled={recoveringK} class="w-full">
					{#if recoveringK}
						<Loader2 class="mr-2 h-4 w-4 animate-spin" />
						Recovering key...
					{:else}

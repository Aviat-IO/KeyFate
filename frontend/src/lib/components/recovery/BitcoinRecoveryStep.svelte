	...
	async function fetchAndDecryptFromBitcoin() {
		...
		try {
			...
			try {
				...
				if (!event) {
					throw new Error(`Nostr event ${nostrEventId.slice(0, 16)}... not found on any relay`);
				}
...
<div class="space-y-6">
	...
	<Button onclick={parseBitcoinTx} disabled={!txHexInput.trim() || parsingTx} class="w-full">
		{#if parsingTx}
	...
	{#if parsedOpReturn}
		...
		<div class="space-y-3">
			...
			<Button onclick={fetchAndDecryptFromBitcoin} disabled={fetchingNostrEvent} class="w-full">

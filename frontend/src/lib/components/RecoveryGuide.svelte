		...
		<Card.Content class="space-y-6">
			...
			<div>
				...
				<ul class="text-muted-foreground space-y-1 text-sm">
					...
					<li>
						&bull; <strong>User-managed shares</strong> — shares stored in your browser (if available)
					</li>
					<li>
						&bull; <strong>Bitcoin data</strong> — pre-signed transaction, UTXO details, timelock info
					</li>
					<li>
						&bull; <strong>Nostr data</strong> — relay list and encrypted key passphrase bundle
					</li>
					<li>
						&bull; <strong>Recovery instructions</strong> — how to reconstruct the secret offline
					</li>
				</ul>
			</div>
				...
				<Accordion.Root type="single" class="w-full">
						...
						<Accordion.Content>
							<div class="space-y-3 pb-2 text-sm">
								<p class="text-muted-foreground">
									The symmetric key K is embedded in a Bitcoin transaction's OP_RETURN output. After
									the CSV timelock expires (owner missed check-in), the pre-signed transaction
								...
								<div class="border-destructive/30 bg-destructive/5 rounded-lg border p-3">
					...
					<Accordion.Item value="nostr">
						...
						<Accordion.Content>
							<div class="space-y-3 pb-2 text-sm">
								<p class="text-muted-foreground">
									K was encrypted using NIP-44 (ECDH + ChaCha20) with the owner's private key and
									your public key. If you have your Nostr private key, you can decrypt K directly.
								</p>
								<ol class="text-muted-foreground list-inside list-decimal space-y-2">
									<li>
										Query the Nostr relays listed in your recovery kit for gift-wrapped events (kind
										1059) tagged with your public key.
									</li>
									<li>
								...
								<div class="border-destructive/30 bg-destructive/5 rounded-lg border p-3">
									<div class="flex gap-2">
										...
										<p class="text-muted-foreground text-xs">
											...
											<strong>not quantum-safe</strong>. Use the passphrase or Bitcoin path for
											long-term security.
										</p>
									</div>
								</div>
						...
						<Accordion.Content>
							<div class="space-y-3 pb-2 text-sm">
								<p class="text-muted-foreground">
									...
									AES-256-GCM. If you know the passphrase, you can recover K without any network
									access.
								</p>
								<ol class="text-muted-foreground list-inside list-decimal space-y-2">
									<li>
										Locate the <code class="bg-muted rounded px-1">encryptedKPassphrase</code> field
										in your recovery kit's Nostr section. It contains
										<code class="bg-muted rounded px-1">ciphertext</code>,
										<code class="bg-muted rounded px-1">nonce</code>, and
										<code class="bg-muted rounded px-1">salt</code> (all hex-encoded).
									</li>
									<li>
										Re-derive the AES key from the passphrase using PBKDF2-SHA256 with the stored
										salt (600,000 iterations).
									</li>
									<li>
										Decrypt the ciphertext with AES-256-GCM using the derived key and stored nonce.
										The result is the 32-byte symmetric key K.
									</li>
									<li>Use K to decrypt each share with ChaCha20-Poly1305.</li>
									<li>Combine the decrypted Shamir shares to reconstruct the secret.</li>
								</ol>
								<div class="border-primary/50 bg-primary/10 rounded-lg border p-3">
									<div class="flex gap-2">
										<Lock class="text-primary h-4 w-4 shrink-0" />
										<p class="text-muted-foreground text-xs">
											This path uses only symmetric cryptography (PBKDF2 + AES-256-GCM +
											ChaCha20-Poly1305). It is fully quantum-safe and works completely offline.
										</p>
									</div>
								</div>
							</div>
			...
			<div>
				...
				<div class="text-muted-foreground space-y-2 text-sm">
					<p>Your recovery kit is designed to work without KeyFate. Here's what to do:</p>
					<ol class="list-inside list-decimal space-y-1">
						<li>
							<strong>Gather shares:</strong> You need at least the threshold number of Shamir shares.
							Check your recovery kit for the server share and any user-managed shares.
						</li>
						<li>
							<strong>Recover K:</strong> Use any of the three paths above (Bitcoin, Nostr, or Passphrase)
							to obtain the symmetric key K.
						</li>
						<li>
							<strong>Decrypt shares:</strong> Each share is encrypted with ChaCha20-Poly1305 using K.
							Decrypt each one using the nonce stored alongside it.
						</li>
						<li>
							<strong>Reconstruct:</strong> Use a Shamir Secret Sharing library to combine the decrypted
							shares. Any implementation of GF(256) Shamir will work.
						</li>
					</ol>
					<p class="mt-2 text-xs italic">
						Tools: You can use the open-source <code class="bg-muted rounded px-1"
							>shamirs-secret-sharing</code
						> npm package, or any compatible implementation. The recovery kit JSON format is documented
						and stable.
					</p>
				</div>
			</div>

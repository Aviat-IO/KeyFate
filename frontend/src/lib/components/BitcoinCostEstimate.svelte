	...
	<Card.Root>
		...
		<Card.Content class="space-y-4">
			...
			<div class="bg-muted/50 rounded-lg p-3">
				<h4 class="mb-2 text-xs font-semibold tracking-wide uppercase">Parameters</h4>
			...
			<!-- Fee breakdown -->
			<div>
				<h4 class="mb-2 text-xs font-semibold tracking-wide uppercase">Fee Breakdown</h4>
			...
			<!-- Annual cost -->
			<div>
				<h4 class="mb-2 text-xs font-semibold tracking-wide uppercase">Annual Cost</h4>
				<div class="space-y-2 text-sm">
					<div class="flex justify-between">
			...
			<!-- UTXO longevity -->
			<div>
				<h4 class="mb-2 text-xs font-semibold tracking-wide uppercase">UTXO Longevity</h4>
				<div class="space-y-2 text-sm">
					<div class="flex justify-between">
			...
			<div class="space-y-2 text-xs">
				<div class="flex items-start gap-2">
					...
					<p class="text-muted-foreground">
						<strong>Minimum UTXO:</strong>
						{formatSats(MIN_UTXO_SATS)} sats. The UTXO must stay above this threshold after each refresh.
						When depleted, add more funds.
					</p>
				</div>
				<div class="flex items-start gap-2">
					<Bitcoin class="text-muted-foreground mt-0.5 h-3 w-3 shrink-0" />
					<p class="text-muted-foreground">
						<strong>Fee variability:</strong> Bitcoin fees fluctuate with network demand. During high-fee
						periods, refresh costs may be 10-100x higher. Consider using a higher initial UTXO amount
						for safety margin.
					</p>
				</div>
				<div class="flex items-start gap-2">
					<Bitcoin class="text-muted-foreground mt-0.5 h-3 w-3 shrink-0" />
					<p class="text-muted-foreground">
						<strong>Refresh deducts fees:</strong> Each check-in refresh deducts the transaction fee from
						the UTXO balance. The UTXO shrinks over time.
					</p>
				</div>
			</div>
		</Card.Content>

...
<nav
	class="bg-background/95 supports-[backdrop-filter]:bg-background/60 border-border/50 border-b backdrop-blur"
>
		...
		<div class="flex h-16 items-center justify-between">
			<div class="flex items-center space-x-4">
				...
				<div class="hidden items-center space-x-1 md:flex">
					{#if user}
						<a
							href="/dashboard"
							class="hover:bg-accent hover:text-accent-foreground rounded-md px-3 py-2 text-sm font-medium transition-colors {isActive(
								'/dashboard'
							)
			...
			<div class="flex items-center space-x-2">
				<Button
					...
					size="sm"
					href="/decrypt"
					class="hidden md:flex {isActive('/decrypt') ? 'bg-accent text-accent-foreground' : ''}"
				>
					Recover Secret
				</Button>
				...
				{#if user}
					...
					<div class="relative hidden md:block">
						...
						{#if mobileMenuOpen}
							...
							<div class="fixed inset-0 z-40" onclick={() => (mobileMenuOpen = false)}></div>
							<div
								class="bg-popover absolute right-0 z-50 mt-2 w-[200px] rounded-md border p-2 shadow-md"
							>
								{#if isAdmin}
									<a
										href="/admin"
		...
		{#if mobileMenuOpen}
			<div class="border-t pb-4 md:hidden" data-testid="dropdown-content">
				<div class="space-y-1 pt-2">
					...
					<a
						href="/decrypt"
						class="{menuItemClass} {isActive('/decrypt') ? 'bg-accent text-accent-foreground' : ''}"
					...
					{#if !user}
						<a href="/auth/signin" class={menuItemClass} onclick={() => (mobileMenuOpen = false)}>
							Sign In
						</a>
					{/if}
				</div>

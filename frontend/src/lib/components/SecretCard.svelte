<script lang="ts">
	...
	function getStatusBadge(
		...
		if (status === 'failed') {
			return {
				label: 'Failed',
				colorClass: 'border-destructive/50 bg-destructive/10 text-destructive'
			};
		}
		if (shareDeleted) {
		...
		if (daysUntilCheckIn <= 5)
			return { label: 'Upcoming', colorClass: 'border-warning/50 bg-warning/10 text-warning' };
...
<div class="py-10">
	...
	<div class="flex items-start justify-between gap-4">
		...
		<Badge
			variant="outline"
			class="shrink-0 text-xs tracking-wider uppercase {statusBadge.colorClass}"
		>
			{statusBadge.label}
		</Badge>
	</div>

<script lang="ts">
	...
	let activeSecrets = $derived(secrets.filter((s) => s.status !== 'triggered' && !s.triggeredAt));
	let sentSecrets = $derived(secrets.filter((s) => s.status === 'triggered' || s.triggeredAt));
...
{#if sentSecrets.length > 0}
	<div class="mt-16">
		...
		<div class="divide-border/50 mt-4 divide-y">

			...
			<Alert.Root variant="default" class="border-accent bg-accent/50">
				...
				<Alert.Description class="space-y-2">
					...
					<Textarea
						...
						readonly
						rows={4}
						class="bg-muted mt-2 w-full rounded-md border p-3 font-mono text-sm select-all"
					/>
					<Button
						type="button"
		...
		<div class="flex items-start space-x-3">
			...
			<div class="space-y-2 text-sm">
				<p class="text-muted-foreground">
					<strong>Educational demonstration:</strong> This tool runs entirely in your browser. Your shares
					are processed locally and never sent to servers. For maximum security with sensitive secrets,
					we recommend running this tool locally on your own device.
				</p>
				<a href="/local-instructions" class="text-muted-foreground h-auto p-0 text-xs underline">

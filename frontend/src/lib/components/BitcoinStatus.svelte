	...
	async function handleRefresh() {
		...
		if (!ownerKeypair) {
			toast.error('Owner keypair not found in session', {
				description:
					'Bitcoin keys are only available in the browser session where they were created. You may need to re-enable Bitcoin for this secret.'
			});
			return;
		}
		...
		try {
			...
			if (!storedMeta) {
				toast.error('Bitcoin metadata not found in session', {
					description:
						'The symmetric key and Nostr event ID are required for refresh. These are only available in the session where Bitcoin was initially set up.'
				});
				return;
			}
...
<Card.Root>
	...
	{#if canRefresh}
		<Card.Footer>
			<Button variant="outline" onclick={handleRefresh} disabled={refreshing} class="w-full">
				{#if refreshing}

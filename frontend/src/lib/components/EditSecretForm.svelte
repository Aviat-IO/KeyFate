...
<form onsubmit={handleSubmit} class="space-y-8">
		...
		<div class="space-y-3">
			{#each recipients as recipient, index}
				<div class="border-border/50 space-y-3 rounded-md border p-4">
					...
					<div class="space-y-2">
						<Label
							for="edit-recipient-{index}-name"
							class="text-muted-foreground text-xs font-medium">Name</Label
						>
						<Input
							id="edit-recipient-{index}-name"
					...
					<div class="space-y-2">
						<Label
							for="edit-recipient-{index}-email"
							class="text-muted-foreground text-xs font-medium">Email</Label
						>
						<Input
					...
					<div class="space-y-2">
						<Label
							for="edit-recipient-{index}-phone"
							class="text-muted-foreground text-xs font-medium">Phone (optional)</Label
						>
						<Input
							id="edit-recipient-{index}-phone"
							type="tel"
	...
	<div class="space-y-4 pt-8">
		...
		<div class="space-y-2">
			...
			{:else}
				<Select.Root
					type="single"
					value={String(checkInDays)}
					onValueChange={(v) => (checkInDays = Number(v))}
				>
					<Select.Trigger>
	...
	<!-- Action Buttons -->
	<div class="flex flex-col justify-between space-y-3 pt-6 sm:flex-row sm:space-y-0 sm:space-x-4">
		<Button
			type="button"
			variant="destructive"

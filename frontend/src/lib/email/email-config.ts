 ...
 */
export function validateEmailProviderVars(provider: EmailProviderType): ValidationResult {
	const errors: string[] = [];
...
export function validateEmailEnvironment(): EmailEnvironmentValidation {
	...
	// Determine provider
	const envProvider = process.env.EMAIL_PROVIDER?.toLowerCase() as EmailProviderType | undefined;
...
export function validateEmailConfigAtStartup(): void {
	...
	if (!validation.valid) {
		...
		console.log(`[EmailConfig] Email configuration valid (${validation.provider} provider)`);
	}
}

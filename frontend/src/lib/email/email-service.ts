...
===================================
      `);
		...
		return {
			...
			error: errorMessage,
			retryable:
				!errorMessage.includes('Invalid API key') && !errorMessage.includes('Authentication failed')
		};
	}
}
...
export async function sendReminderEmail(reminderData: {
	...
	const result = await sendEmail({
		...
		text: template.text,
		priority:
			reminderData.urgencyLevel === 'critical' || reminderData.urgencyLevel === 'high'
				? 'high'
				: 'normal',
		trackDelivery: true
	});
...
class BillingEmailService {
	async sendSubscriptionConfirmation(userId: string, data: SubscriptionConfirmationData) {
		try {
			...
			const { renderSubscriptionConfirmationTemplate } = await import('./templates');
	...
	async sendSubscriptionCancelledNotification(userId: string) {
		try {
			...
			const { renderSubscriptionCancelledTemplate } = await import('./templates');
	...
	async sendBitcoinPaymentConfirmation(userId: string, data: BitcoinPaymentData) {
		try {
			...
			const { renderBitcoinPaymentConfirmationTemplate } = await import('./templates');
			...
			logger.error(
				'Failed to send Bitcoin payment confirmation',
				error instanceof Error ? error : undefined
			);
		}
	}

...
export class DeadLetterQueue {
	...
	async getStats(): Promise<DeadLetterStats> {
		...
		const { classifyFailure, getRetryLimitForEmailType } = await import('./email-retry-service');
		const permanent = unresolved.filter((f) => classifyFailure(f.errorMessage) === 'permanent');
	 ...
	 */
	async getRecipientFailures(recipient: string, limit: number = 10): Promise<EmailFailure[]> {
		const db = await getDatabase();
	...
	async getFailuresByType(
		...
		return failures.map((failure) => {
			...
			const canRetry =
				!failure.resolvedAt && classification === 'transient' && failure.retryCount < retryLimit;

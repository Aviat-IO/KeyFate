 ...
 */
export async function logEmailFailure(failureData: EmailFailureInsert): Promise<EmailFailure> {
 ...
 */
export async function incrementRetryCount(failureId: string): Promise<EmailFailure> {
	const db = await getDatabase();
 ...
 */
export async function resolveEmailFailure(failureId: string): Promise<EmailFailure> {
	const db = await getDatabase();
 ...
 */
export async function getUnresolvedFailures(limit: number = 100): Promise<EmailFailure[]> {
	const db = await getDatabase();
	return await db.select().from(emailFailures).where(isNull(emailFailures.resolvedAt)).limit(limit);
}
 ...
 */
export async function cleanupOldFailures(retentionDays: number = 30): Promise<number> {
	const db = await getDatabase();

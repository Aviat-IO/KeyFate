...
export class StripeProvider implements PaymentProvider {
	...
	async verifyWebhookSignature(
		...
		const event = await this.stripe.webhooks.constructEventAsync(payload, signature, secret);
	...
	private mapStripeSubscription(subscription: Stripe.Subscription): Subscription {
		...
		const periodStart = (firstItem as unknown as Record<string, unknown>).current_period_start as
			| number
			| undefined;
		const periodEnd = (firstItem as unknown as Record<string, unknown>).current_period_end as
			| number
			| undefined;
		return {
			id: subscription.id,

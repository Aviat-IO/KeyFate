...
export class BTCPayProvider implements PaymentProvider {
	...
	async createCustomer(_email: string, _metadata?: Record<string, string>): Promise<string> {
		...
		const customerId = `btcpay_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
	...
	async createSubscription(customerId: string, config: SubscriptionConfig): Promise<Subscription> {
		...
		const subscriptionId = `sub_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
		const now = new Date();
		return {
			id: subscriptionId,
			customerId,
	...
	async listPrices(_productId?: string): Promise<Price[]> {
		...
		const yearlyAmount = isTest ? BTCPayProvider.TEST_YEARLY_BTC : BTCPayProvider.PROD_YEARLY_BTC;
	...
	async convertToProviderCurrency(amount: number, fromCurrency: string): Promise<number> {
		...
		try {
			...
			throw new Error(`Failed to get exchange rate for ${fromCurrency}: ${error}`);
		}
	}
	...
	async getInvoice(invoiceId: string): Promise<BTCPayInvoice> {
		try {
			...
			if (!response.ok) {
				...
				throw new Error(`Failed to fetch BTCPay invoice (${response.status}): ${errorText}`);
			}
	...
	private mapInvoiceToPayment(invoice: BTCPayInvoice): Payment {
		return {
			...
			description: (invoice.metadata?.description as string | undefined) || undefined,
			metadata: (invoice.metadata as Record<string, string> | undefined) || undefined,

...
export function createCSVTimelockScript(
	...
	if (ttlBlocks < 1 || ttlBlocks > MAX_CSV_BLOCKS) {
		throw new Error(`ttlBlocks must be between 1 and ${MAX_CSV_BLOCKS}, got ${ttlBlocks}`);
	}
...
export function decodeCSVTimelockScript(script: Uint8Array): {
	...
	if (decoded.length !== 10) {
		throw new Error(`Invalid CSV timelock script: expected 10 elements, got ${decoded.length}`);
	}
	if (decoded[0] !== 'IF') throw new Error('Script must start with IF');
	if (decoded[2] !== 'CHECKSIG') throw new Error('Expected CHECKSIG after owner pubkey');
	...
	if (decoded[8] !== 'CHECKSIG') throw new Error('Expected CHECKSIG after recipient pubkey');
 ...
 */
export function createOpReturnPayload(symmetricKeyK: Uint8Array, nostrEventId: string): Uint8Array {
	if (symmetricKeyK.length !== 32) {
		throw new Error(`Symmetric key must be 32 bytes, got ${symmetricKeyK.length}`);
	}
	if (nostrEventId.length !== 64) {
		throw new Error(`Nostr event ID must be 64 hex chars, got ${nostrEventId.length}`);
	}

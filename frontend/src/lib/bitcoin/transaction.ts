...
export function createTimelockUTXO(params: {
	...
	if (amountSats < MIN_UTXO_SATS) {
		throw new Error(`Amount ${amountSats} sats is below minimum ${MIN_UTXO_SATS} sats`);
	}

	// Create the timelock script
	const timelockScript = createCSVTimelockScript(ownerPubkey, recipientPubkey, ttlBlocks);

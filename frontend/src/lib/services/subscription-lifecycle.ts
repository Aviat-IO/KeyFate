...
export async function createSubscription(data: CreateSubscriptionData) {
	...
	try {
		...
		logger.error('Failed to create subscription', error instanceof Error ? error : undefined);
		throw error;
	}
}
 ...
 */
export async function updateSubscription(userId: string, data: UpdateSubscriptionData) {
	const db = await getDatabase();
	try {
		...
		logger.error('Failed to update subscription', error instanceof Error ? error : undefined);
		throw error;
	}
}
...
export async function cancelSubscription(userId: string, immediate: boolean = false) {
	try {
		...
		sendCancellationEmail(userId).catch((error) => {
			logger.error('Failed to send cancellation email', error instanceof Error ? error : undefined);
		});
...
export async function getUserSubscription(userId: string) {
	...
	try {
		...
		logger.error('Failed to get user subscription', error instanceof Error ? error : undefined);
		throw error;
	}
}
...
export async function hasActiveSubscription(userId: string): Promise<boolean> {
	...
	try {
		...
		logger.error('Failed to check active subscription', error instanceof Error ? error : undefined);
		return false;
	}
}
 ...
 */
export async function updateUserTier(userId: string, tierName: SubscriptionTier) {
	try {
		...
		logger.error('Failed to update user tier', error instanceof Error ? error : undefined);
		throw error;
	}
}
 ...
 */
export async function getSubscriptionsByProvider(provider: SubscriptionProvider) {
	const db = await getDatabase();
	try {
		return await db
			.select()
 ...
 */
export async function handlePaymentFailure(userId: string, attemptCount: number = 1) {
	try {
		...
		logger.error('Failed to handle payment failure', error instanceof Error ? error : undefined);
		throw error;
	}
}
...
export async function scheduleDowngrade(userId: string) {
	...
	try {
		...
		logger.error('Failed to schedule downgrade', error instanceof Error ? error : undefined);
		throw error;
	}
}

...
export async function getTierByName(tierName: SubscriptionTier) {
	...
	try {
		...
		logger.error('Failed to get tier by name', error instanceof Error ? error : undefined, {
			tierName
		});
		throw error;
	}
...
async function ensureTierExists(tierName: SubscriptionTier) {
	...
	try {
		...
		logger.error(`Failed to create tier ${tierName}`, error instanceof Error ? error : undefined);
		throw error;
	}
}
...
export async function getTierById(tierId: string) {
	...
	try {
		...
		logger.error('Failed to get tier by ID', error instanceof Error ? error : undefined, {
			tierId
		});
		throw error;
	}

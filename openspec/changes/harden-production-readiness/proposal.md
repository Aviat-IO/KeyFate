...
- **BREAKING**: replace raw-secret Shamir splitting and self-anchored Nostr manifests with authenticated v3 recovery: AEAD-encrypt the secret, split only a random content key, require an owner-delivered recipient setup bundle, restrict automated service/Nostr recovery to threshold 2 with one shared logical recipient share, and isolate legacy interpolation behind an explicit unverified mode.
...
- Enforce CSP, prohibit recovery shares in URLs and generated `mailto:` links, update the already-flagged sanitizer, add explicit liveness/readiness probes, separate migrations from app startup, slim/pin the production image, and make CI/release gating auditable.
...
- Affected code: browser share/recovery flows, authenticated share envelopes and recipient setup bundles, Nostr and Bitcoin modules, secret/recovery APIs, cron workers, GDPR export, payment routes/webhooks, auth/rate limiting, Drizzle schema/migrations, SvelteKit CSP/health, Docker, GitHub Actions, and Railway runbooks.
